...
public class CusBitmapUtils {
    ...
    public static Bitmap getDataFromAssets(Context context,String fileName){
        Log.i("CusBitmapUtils","准备从assets文件夹中读取文件"+fileName);
        try {
            ...
            Log.i("CusBitmapUtils","要读取的文件的长度是"+fileSize);//注意这个地方如果文件大小太大，在decodeStream需要设置参数进行裁剪
...
//            Bitmap bitmap = BitmapFactory.decodeFileDescriptor(fileDescriptor);//注意，AssetFileDescriptor只能用来获取文件的大小，不能用来获取inputStream，用FileDescriptor获取的输入流BitmapFactory.decodeStream不能识别
            if(bitmap==null)Log.i("CusBitmapUtils","decode bitmap失败");
            return bitmap;
        } catch (Exception e) {
            Log.i("CusBitmapUtils","读取文件出现异常",e);
            e.printStackTrace();
        }
        return null;
    }
    ...
    public static long getPhoneTotalMemory() {
        ...
        try {
            ...
            for (String num : arrayOfString) {
                Log.i("CusBitmapUtils",str2+"内存是::::"+num + "\t");
            }
            initial_memory = Integer.valueOf(arrayOfString[1]).intValue();// 获得系统总内存，单位是KB，乘以1024转换为Byte
            localBufferedReader.close();
        } catch (IOException e) {
            Log.i("CusBitmapUtils","获取系统总内存出现异常",e);
        } catch (Exception e){
            Log.i("CusBitmapUtils","获取系统总内存出现异常2",e);
        }
    ...
    public static Bitmap tiny2BlurBitmap(Context context,Bitmap tinyBitmap){
        ...
        Bitmap bigBitmap = CusBitmapUtils.scaleBitmap(tinyBitmap, scaleRatio, scaleRatio);
        tinyBitmap.recycle();//微型图片已经没用了，回收掉
        //将300宽度的大图进行高斯模糊，模糊半径为8
        Bitmap blurBitmap = CusBitmapUtils.fastblur(context, bigBitmap, 8);
