...
public class PicassoUtils {
    ...
    private static CusPicassoOk3Downloader.ProgressListener progressListener;
    //用于管理进度条的map,使用弱引用可以防止OOM
   ...
   //用于管理进度条的map,使用弱引用可以防止OOM
     ...
     * @return
     */
    static private  Picasso getPicasso(Context context, CusPicassoOk3Downloader.ProgressListener listener){
        OkHttpClient client=CusOK3ClientManager.getDefaultClient(context);
        CusPicassoOk3Downloader downloader=new CusPicassoOk3Downloader(client,listener);
        if(picasso==null)
    ...
    private static CusPicassoOk3Downloader.ProgressListener getListener(){
        if(progressListener == null)progressListener = new CusPicassoOk3Downloader.ProgressListener() {
            @Override
                    ...
                    public void run() {
                        ...
                        if(oldPregress > percent){//正常情况下，每次的percent都应该比上次大
                            ...
                            if(oldPregress == 100) {
                                ...
                                Log.i("PicassoUtils","现在已经成功显示出来了，不需要再下载了");
                        ...
                        if(oldPregress == 100){
                            Log.i("PicassoUtils","奇怪，以前不是成功了么？");
                        }
                        if(progressWheel.getVisibility() == View.GONE)progressWheel.setVisibility(View.VISIBLE);
                        if(textView.getVisibility() == View.GONE)textView.setVisibility(View.VISIBLE);
    ...
    public static void displayImageProgress(final String url, ImageView imageView , final ProgressWheel progressWheel, final TextView textView, final String base64Blur){
        ...
        if(oldProgress == null) {//null为以前没有下载过，0为开始下载但是没收到数据
            Log.i("PicassoUtils","当前图片未下载");
            progressWheel.setVisibility(View.GONE);
            textView.setVisibility(View.GONE);
            progressHashMap.put(url,0);
        }else if(progressHashMap.get(url) >= 100){
            Log.i("PicassoUtils","当前图片下载已经完成"+progressHashMap.get(url));
            progressWheel.setVisibility(View.GONE);
            textView.setVisibility(View.GONE);
        } else {
            Log.i("PicassoUtils","当前图片正在下载");
        ...
        if(oldProgress == null || oldProgress < 100){//如果没开始下载或者没有下载完
            ...
            if(!TextUtils.isEmpty(base64Blur)) blurBitmap = CusBitmapUtils.base64ToBlurBitmap(imageView.getContext(),base64Blur);
