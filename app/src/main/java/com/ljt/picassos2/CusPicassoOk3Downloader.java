...
public final class CusPicassoOk3Downloader implements Downloader{

    private final Call.Factory client;
    private final Cache cache;


    public CusPicassoOk3Downloader(OkHttpClient client,final ProgressListener listener) {
        this.client = client.newBuilder().addNetworkInterceptor(new Interceptor() {
            @Override public okhttp3.Response intercept(Chain chain) throws IOException {
                okhttp3.Response originalResponse = chain.proceed(chain.request());
                return originalResponse.newBuilder().body(
                        new CusPicassoOk3Downloader.ProgressResponseBody(originalResponse.body(),originalResponse.request().url().url().toString(), listener))
                        .build();
            }
        }).build();
    ...
    public CusPicassoOk3Downloader(Call.Factory client) {
        this.client = client;
        this.cache = null;
    }
    public CusPicassoOk3Downloader(OkHttpClient client) {
        this.client = client;
    ...
    @Override public Response load(Uri uri, int networkPolicy) throws IOException {
        Log.i("CusPicassoOk3Downloader","准备从php拿url去cdn要图片的uri是->"+uri+"    缓存策略是"+networkPolicy);
        ...
        Log.i("CusPicassoOk3Downloader","php响应码是"+php_responseCode);
        ...
        Log.i("CusPicassoOk3Downloader","php的响应是不是从缓存拿的呀？"+fromPhpCache);
        Log.i("CusPicassoOk3Downloader","全部的header是"+php_response.headers());
        if("text/html".equals(php_response.header("Content-Type")) ||
                "text/plain".equals(php_response.header("Content-Type")))
        {//如果php发来的是cdn的图片url
            Log.i("CusPicassoOk3Downloader","现在是从php取得的url字符串而不是jpg");
            String cdnUrl = php_response.body().string();
            Log.i("CusPicassoOk3Downloader","php服务器响应时间"+(System.currentTimeMillis() - startTime));
            Log.i("CusPicassoOk3Downloader","CDN的imageUrl是->"+cdnUrl);
            ...
            Log.i("CusPicassoOk3Downloader","cdn的响应码是"+cdn_responseCode);
            ...
            Log.i("CusPicassoOk3Downloader","cdn响应时间"+(System.currentTimeMillis() - cdnStartTime));
            boolean fromCache = cdn_response.cacheResponse() != null;
            ResponseBody cdn_responseBody = cdn_response.body();
            Log.i("CusPicassoOk3Downloader","cdn的图片是不是从缓存拿的呀？fromCache = "+fromCache);
            ...
            Log.i("CusPicassoOk3Downloader","准备直接用PHP的图片！！！");
